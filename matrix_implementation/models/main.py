...
def get_model_dump_paths(params):
    output_path = MODEL_OUTPUT_DIR.format(params.sample_weight, params.distribution_req, params.algo_type, params.model)
...
def get_dataloader(params, test=False):
    ...
    if test:
        ...
        if params.coreset == 1:
            loc = INPUT_IMG_DIR_CORESET.format(params.dataset, params.sample_weight, params.distribution_req, params.algo_type, params.partitions)
        elif params.coreset == 0:
            loc = INPUT_IMG_DIR_FULLDATA.format(params.dataset, params.sample_weight)
...
def get_model_metric_file(params):
    return METRIC_FILE.format(params.coverage_factor, params.sample_weight, params.distribution_req, params.algo_type, params.dataset)
...
def train_and_test(model, params):
    ...
    # save model
    model_path = os.path.join(checkpoint_path, '{0}.pt'.format(model_id))
...
if __name__=="__main__":
    ...
    parser.add_argument('--num_runs', type=int, default=1, help="number of runs for model testing")
    ...
    parser.add_argument('--sample_weight', type=float,default=1.0)
