...
import numpy as np
from paths import FEATURE_VECTOR_LOC, MODELS
...
def get_all_resnet(all_images, model_name):
...
if __name__ == "__main__":
    ...
    for key, value in MODELS.items():
        if key not in ['resnet', 'alexnet', 'densenet', 'efficientnet_b0', 'efficientnet_b2']:
            print('Generating fvs for {0}'.format(key))
            batch_size = 32
            resnet_vectors = np.ones((len(all_images),value))
            for i in range(0, len(all_images), batch_size):
                resnet_vectors[i : i + batch_size] = get_all_resnet(all_images[i : i + batch_size], key)
            
            
            resnet = np.array(resnet_vectors)
            # print(resnet_vectors.shape)
            location = FEATURE_VECTOR_LOC.format('cifar10', key)
            f = open(location, 'wb')
