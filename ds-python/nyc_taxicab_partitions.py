...
def clean_1_mil_data():
    ...
    df = df.loc[:,~df.columns.str.match("Unnamed")]
    print(df.keys())
    df.to_csv('/localdisk3/nyc_1mil_2021-09.csv', index=False)



def partition_data(p=10):
    df = pd.read_csv('/localdisk3/nyc_1mil_2021-09.csv')
    # print(df.keys())
    ids = df['ID'].tolist()
    # print(ids[0])
    random.shuffle(ids)
    ...
    FILE_LOC = '/localdisk3/data-selection/data/metadata/nyc_taxicab/10/datasets/{0}.csv'
    # df_p0 = df[df['ID'].isin(partitions[0])]
    # print(df_p0.shape)
    for i, part_data in enumerate(partitions):
        df_p = df[df['ID'].isin(part_data)]
        df_p.to_csv(FILE_LOC.format(i))
