...
def gfkc_db(cf, dr, dataset_name, dataset_size, num_classes):
    ...
    GC[0] = np.array(dr)
    while ((not np.all(CC[0] <= 0)) or (not np.all(GC[0] <= 0))) and len(coreset) < dataset_size:
        ...
        for r in row:
            posting_list_best_point[list(r[0])] = 1
...
def test_psql():
    conn = psycopg2.connect(database="pmundra")
    cur = conn.cursor()
    delta = set([i for i in range(50000)])
    coreset = set()
    possible_ids = tuple(delta.difference(coreset))
    psql_time = time.time()
    cur.execute('select id, pl from cifar10_postinglist where id in %s;', (possible_ids, ))
    print('db_query time : {0}'.format(time.time() - psql_time))
